//
//  ZTAddAddressTableViewController.m
//  WifeButler
//
...
@implementation ZTAddAddressTableViewController
...
- (void)downLoadInfoXiangQin
{
    ...
    NSString *url = [HTTP_BaseURL stringByAppendingFormat:@"%@", KShenShiQuAddressOne];
    ...
    [manager POST:url parameters:dic progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if ([responseObject[@"code"] intValue] == 10000) {
            ...
            self.nameTextF.text = responseObject[@"resultCode"][@"realname"];
            self.iphoneTextF.text = responseObject[@"resultCode"][@"phone"];
            self.youBianTextF.text = responseObject[@"resultCode"][@"post"];
            self.address1TextF.text =[NSString stringWithFormat:@"%@%@%@", responseObject[@"resultCode"][@"pro_name"],responseObject[@"resultCode"][@"city_name"],responseObject[@"resultCode"][@"qu_name"]];
            
            self.address2TextV.text = responseObject[@"resultCode"][@"address"];
            self.xiaoQuLab.text = responseObject[@"resultCode"][@"village_name"];
            
            self.xiaoQu_id = responseObject[@"resultCode"][@"village"];
            
            self.sheng_id = responseObject[@"resultCode"][@"pro"];
            self.shi_id = responseObject[@"resultCode"][@"city"];
            self.qu_id = responseObject[@"resultCode"][@"qu"];
            
            if ([responseObject[@"resultCode"][@"defaults"] intValue] == 1) {
                
                self.isMoRenAddress.on = NO;
            }
            else
            {
                self.isMoRenAddress.on = YES;
            }
...
- (void)downLoadInfo
{
    ...
    if (_isMoRenAddress.on == YES) {
        ...
        [dic setObject:@"2" forKey:@"deafults"];
    }
    else// 不是默认地址
    {
        [dic setObject:@"1" forKey:@"deafults"];
    }
    
    NSString *url = [HTTP_BaseURL stringByAppendingFormat:@"%@", KBianJiShouHuoAddress];
